...
const SECTIONS = [
  ...
  },
  {
    ...
    workflow: [
      ...
      "Payment Status on the list is Owing (balance still due), Paid (settled by a fresh payment), or PB — Paid Before (settled at least partly by applying an existing credit). In practice PB/Owing are rare here — a walk-in sale is almost always paid in full on the spot; Guest Folios (room-based stays) is where those statuses mostly come from.",
      "Shows up automatically in the Reports → Dashboard tab's Payments Received / Payments by Method totals, and in Food Sales / Drink Sales — there's no separate walk-in-sales report to check.",
    ],
  },
  {
    id: "check-ins",
  ...
  },
  {
    ...
    workflow: [
      ...
      "Food Sales and Drink Sales both show a Total, a payment-method breakdown (grouped by how each walk-in/walk-in-folio charge was actually paid — a charge still owing isn't in it, since nothing's been paid yet; every folio-billed charge is grouped into one \"Charged to Room\" figure instead, since there's no way to know which later payment actually covered which specific charge), and a By Staff breakdown of who posted each charge.",
      "Remarks (whatever was typed into the Notes field when the charge was posted — see Guest Folios) shows up here too. A Walk-In Sales charge never has one of its own (there's no per-charge notes field there) — legacy and Guest Folios charges are the only sources for this column.",
  ...
  },
  {
    ...
    label: "Menu",
    managerOnly: true,
    summary: "The food and drink item list — name and price — that Guest Folios' Food/Drink Charge picker and Walk-In Sales both pull from.",
    workflow: [
