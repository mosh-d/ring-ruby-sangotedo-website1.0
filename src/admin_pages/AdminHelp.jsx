...
const SECTIONS = [
  ...
  },
  {
    ...
    workflow: [
      ...
      "Food Sales — every food order for a given date, one row per order, whether charged to a room's folio or sold as a walk-in (see Walk-In Sales). Status is Cash/Transfer/etc. for a walk-in sale, Complementary for a $0 folio charge, or the room's own Owing/Paid/PB status otherwise (same three-way split as Accommodation's Payment Status — a payment settles a whole folio, not one specific charge, so this is the folio's overall status, not proof this exact item was paid for).",
      "Drink Sales — every drink sold for a given date, aggregated to one row per item (quantity sold, unit price, total revenue) rather than one row per order — mirrors the old paper bar-analysis sheet's sales columns. Doesn't track physical stock (opening/added/damaged/closing counts) — this app doesn't record drink inventory anywhere, only what sold.",
      "Food Sales and Drink Sales both show a Total, a payment-method breakdown (walk-in sales split by method; every folio-billed charge grouped into one \"Charged to Room\" figure, since there's no way to know which later payment actually covered which specific charge), and a By Staff breakdown of who rang each sale/charge in.",
      "Remarks (whatever was typed into the Remarks field when the charge or sale was posted — see Folios and Walk-In Sales) shows up here too. Drink Sales aggregates by item, so if more than one order behind the same row had its own Remarks, they're all shown together, separated by semicolons.",
