...
const SECTIONS = [
  ...
  },
  {
    ...
    workflow: [
      ...
      "Nights an OTA is paying for are marked at check-in and stay charged here, so the folio keeps reading owing until the OTA remits — but they are left out of what the desk asks the guest for, shown as Guest To Pay. Marking one paid records the money against the folio with the method OTA, automatically. Everything still outstanding is listed on the OTA Payments page.",
  ...
  },
  {
    ...
    workflow: [
      ...
      "Manifest — room sales for the day: one row per room occupied that night (actual occupancy/billing, not scheduled dates — this is the deliberate difference from Accommodation above). A guest who checked out during the day is left off, since their last night was sold the day before. The exception is a guest who checked in and out within the same business day: that room was sold that day, so they stay, marked Checked In & Out. Columns: arrival and check-out dates, room price, breakfast price, payment mode, payment status (Paid / Owing / PB — see Guest Folios for what PB means), Receipt No. (the receipt of the payment that settled that night; NIL while owing), amount paid today, Refund (money handed back to the guest that day, shown as a negative amount; NIL when none), Guest Status (Checked In / Stay Over / Checked In & Out; Stay Over means the guest was already in house the day before) and Remarks (how the booking came in: Walk-in, Website or OTA). The Total Room Revenue line excludes breakfast. Below it, Non-Revenue Rooms lists the rooms someone stayed in that bring in no money: complementary guests, and any room set aside for a manager, shown as Manager since managers are never checked in. Out-of-order rooms are not listed.",
      "Below that table it also carries one Payments section per payment method (a refund shows as a negative amount tagged Refund), then Reservation (Credit) — advance payments taken this business day; Debt Recovery — old debt cleared by money received this business day; and Notes — each listed stay's Special Requests plus its own Notes list. All three moved here from the Accommodation report, and the Notes now follow the rooms listed above them rather than the day's scheduled arrivals.",
      "Food Sales — every food order for a given date, one row per order, whether charged to a room's folio or a non-guest sale. Status is Owing / Paid / PB — Paid Before (same three-way split as Manifest's Payment Status, since a payment settles a whole folio rather than one specific charge) or Complementary for a $0 charge. A separate Payment Method column shows how it was actually paid, when that's known.",
